...
st.info("チャート下に、選択した通貨ペアの最新ポジション分析と、スコアの時系列推移が表示されます。")
...
def analyze_score_history(base_asset, quote_asset, all_cot_data):
    base_df = all_cot_data[all_cot_data['Name'] == base_asset].copy()
    quote_df = all_cot_data[all_cot_data['Name'] == quote_asset].copy()
    ...
    base_series = base_df[['Date', 'NonComm_Idx']].set_index('Date').rename(columns={'NonComm_Idx': 'base_score'})
    quote_series = quote_df[['Date', 'NonComm_Idx']].set_index('Date').rename(columns={'NonComm_Idx': 'quote_score'})
    
    merged_df = pd.merge(base_series, quote_series, on='Date', how='inner')
    merged_df['ペア総合スコア'] = merged_df['base_score'] - merged_df['quote_score']
    merged_df['週次変化'] = merged_df['ペア総合スコア'].diff()
    
    history_df = merged_df[['ペア総合スコア', '週次変化']].dropna().sort_index(ascending=False)
    return history_df
...
def main():
    ...
    try:
        ...
        if base_asset_cot and quote_asset_cot:
            ...
            else: st.warning("スナップショット分析に必要なCOTデータが不足しています。")
            ...
            else: st.warning("時系列分析に必要なCOTデータが不足しています。")
